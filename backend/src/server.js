...
// Important: ensure proxies/caches vary by origin
app.use((req, res, next) => {
...
// -----------------------------
// Rate limiting (doesn't break CORS)
// -----------------------------
const rateLimiter = new RateLimiterMemory({
  points: parseInt(process.env.RATE_LIMIT_MAX_REQUESTS, 10) || 100,
  duration: parseInt(process.env.RATE_LIMIT_WINDOW_SECONDS, 10) || 900, // 15 minutes
});

// Apply rate limiting middleware AFTER CORS
app.use((req, res, next) => {
  rateLimiter
    .consume(req.ip)
    .then(() => next())
...
// -----------------------------
// Socket.IO (uses same origin logic)
// -----------------------------
const io = socketIo(server, {
  cors: {
    origin: (origin, cb) => {
      if (isAllowedOrigin(origin)) return cb(null, true);
      return cb("Not allowed by CORS", false);
    },
    credentials: true,
    methods: ["GET", "POST"],
  },
...
// Make io and logger accessible to routes
app.set("socketio", io);
app.set("logger", logger);
...
// Optional: detailed health (keep if your frontend checks it)
app.get("/health/detailed", async (req, res) => {
  res.json({
    ok: true,
    dbConfigured: !!process.env.MONGODB_URI,
    tradierConfigured: !!process.env.TRADIER_API_KEY && process.env.TRADIER_API_KEY !== "your_tradier_api_key_here",
...
const connectDB = async () => {
  ...
  if (uri && uri !== "mongodb://localhost:27017/options_analysis") {
    try {
